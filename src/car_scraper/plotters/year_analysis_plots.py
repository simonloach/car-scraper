...
class YearAnalysisPlotter:
    ...
    def generate_year_analysis_plots(self, model: Optional[str] = None) -> None:
        ...
        if 'internal_id' in df.columns:
            ...
            logger.info(f"Deduplicated from {original_count} entries to {len(df)} unique cars")
            click.echo(f"Deduplicated from {original_count} entries to {len(df)} unique cars")
        else:
            logger.warning("No internal_id column found, analysis may include duplicates")
    ...
    def _generate_four_panel_analysis(self, df: pd.DataFrame, model: Optional[str] = None) -> None:
        ...
        year_plot_file = self.plots_dir / f'year_analysis{"_" + model.replace("/", "_") if model else ""}.png'
    ...
    def _generate_year_scatter_plot(self, df: pd.DataFrame, model: Optional[str] = None) -> None:
        ...
        year_scatter_file = self.plots_dir / f'listings_by_year{"_" + model.replace("/", "_") if model else ""}.png'
    ...
    def _generate_price_vs_mileage_plot(self, df: pd.DataFrame, model: Optional[str] = None) -> None:
        ...
        if len(valid_data) > 0:
            ...
            price_mileage_file = self.plots_dir / f'price_vs_mileage{"_" + model.replace("/", "_") if model else ""}.png'
    ...
    def _print_year_analysis_summary(self, df: pd.DataFrame, model: Optional[str] = None) -> None:
