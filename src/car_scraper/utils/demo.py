...
class DemoRunner:
    ...
    def simulate_price_changes(self, change_count: int = 5) -> bool:
        ...
        valid_listings = {}
        for entry in data:
            if (entry.get('price') and entry.get('price') > 0 and 
                entry.get('id') and entry['id'] not in valid_listings):
                valid_listings[entry['id']] = entry
        
        if len(valid_listings) < change_count:
            change_count = len(valid_listings)
            logger.warning(f"Reducing change count to {change_count} (available listings)")
        ...
        for listing_id in selected_ids:
            ...
            logger.info(f"Simulated change for {listing_id[:20]}...: "
        ...
        try:
            ...
            logger.success(f"Simulated {change_count} price changes for date {new_date}")
            return True
    ...
    def create_sample_data(self, model: str = "lexus-lc", count: int = 10) -> bool:
        ...
        for i in range(count):
            ...
            for days_ahead in range(7, 30, 7):  # Weekly entries
                ...
                historical_entry['price_change_percent'] = (new_price - base_price) / base_price * 100
    ...
    def run_complete_demo(self, model: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Found {len(status_info.get('model_files', []))} model data files")
            ...
            logger.info(f"Demo completed with {len(final_status.get('model_files', []))} data files")
            
            logger.success("✅ Demonstration completed successfully!")
            logger.info("🔍 Check the plots in 'data/plots/' to see individual listing price trends")
    ...
    def assign_internal_ids_to_existing_data(self) -> bool:
        ...
        try:
            ...
            unique_ids = len(set(entry.get('internal_id') for entry in updated_data 
