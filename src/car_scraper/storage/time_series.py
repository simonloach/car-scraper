...
class TimeSeriesStorage:
    ...
    def update_historical_data(
        self, new_data: List[Dict], output_format: str = "json"
    ) -> None:
        """
    ...
    def clean_duplicates(self, dry_run: bool = True) -> None:
        ...
        snapshot_files = list(self.time_series_dir.glob("*.json")) + list(
            self.time_series_dir.glob("*.csv")
        )
        snapshot_files = [
            f for f in snapshot_files if not f.name.startswith("historical")
        ]
