...
class IndividualListingsStorage:
    ...
    def store_listings_data(self, date_str: str, output_format: str = 'json') -> None:
        ...
        if not model_files:
            logger.warning("No model data files found. Skipping individual listings storage.")
        ...
        updated_data = self._update_listings_history(existing_data, all_listings, date_str)
        
        # Save updated historical data
        self._save_historical_data(updated_data, historical_file, output_format)
        
    def _load_existing_data(self, historical_file: Path, output_format: str) -> List[Dict]:
        """Load existing historical data"""
        existing_data = []
    ...
    def _save_historical_data(self, updated_data: List[Dict], historical_file: Path, output_format: str) -> None:
        """Save updated historical data"""
        try:
            if output_format == 'json':
    ...
    def _update_listings_history(self, existing_data: List[Dict], new_listings: List[Dict], date_str: str) -> List[Dict]:
        """
    ...
    def get_historical_data(self, model: Optional[str] = None) -> pd.DataFrame:
        ...
        if historical_file_csv.exists():
            ...
            raise FileNotFoundError("No historical listings data found. Run scraping first.")
